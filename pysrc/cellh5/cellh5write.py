...
class CH5ImageChannelDefinition(CH5PositionDescription):
    fields = ['channel_name', 'description', 'is_physical',
              'voxel_size', 'color']
...
class CH5FileWriter(CH5File):
    ...
    def add_position(self, coord):
        path_to_grp = "/%s/%s/%s/%s/%s/%s/%s" \
                      %(CH5Const.PREFIX, CH5Const.PLATE, "%s", CH5Const.WELL,
                        "%s", CH5Const.SITE, "%s")
...
class CH5ImageWriter(CH5PositionWriterBase):
    def __init__(self, dset, order, parent_pos):
        ...
        self.image_wide_object_writer = CH5ImageWideObjectWriter(
            os.path.split(self.dset.name)[1], obj_root_grp, self.parent_pos)
...
class CH5RegionWriter(CH5ObjectWriter):
    ...
    def write(self, t, object_labels):
        ...
        self.dset[self.offset:self.offset+len(object_labels)] = \
            (numpy.c_[times, object_labels]).astype(numpy.int32).view(
                dtype=self.dtype).T
...
class CH5FeatureCompoundWriter(CH5PositionWriterBase):
    ...
    def write(self, data):
        ...
        self.dset[self.offset:self.offset+len(data)] = \
            data.astype(numpy.int32).view(dtype=self.dtype)[:,0]
...
class CH5ImageWideObjectWriter(CH5ObjectWriter):
    ...
    def write(self, t, c, z):
        ...
        self.dset[self.offset:self.offset+1] = \
                numpy.array([self.offset, t,c,z], dtype=numpy.int32).view(
                    dtype=self.dtype).T
...
if __name__ == "__main__":
    ...
    for c in range(2):
        for t in range(10):
            for z in range(1):
                image = raw[c, t, z, :, :]
                ciw.write(image, c=c, t=t, z=z)

    ciw.finalize()

    c_def = CH5ImageChannelDefinition()

    c_def.add_row(channel_name='1', description='rfp', is_physical=True,
                  voxel_size=(1,1,1), color="#aabbcc")
    c_def.add_row(channel_name='2', description='gfp', is_physical=True,
                  voxel_size=(1,1,1), color="#aabbcc")
    ...
    cfew = cpw.add_object_feature_matrix(
        object_name='seg c 2', feature_name="object_features", n_features=239,
        dtype=numpy.float32)
    ...
    cfew = cpw.add_object_feature_matrix(
        object_name='seg c 1', feature_name="object_features",
        n_features=123, dtype=numpy.float32)
